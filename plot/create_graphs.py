...
def normalize_dataset(data: pd.DataFrame, mpi_lib : str, base : str | None = None) -> pd.DataFrame:
    ...
    if base is None:
        if mpi_lib in ['OMPI', 'OMPI_SWING']:
            ...
            base = 'default_mpich'
        elif mpi_lib in ['FJMPI']:
            base = 'default-default'
            #base = "SWING-BINOMIAL_TREE-UTOFU-0-1"
...
def main():
    ...
    parser.add_argument("--summarize", required=False, help="Summarizes algorithms (e.g., all Swings reported as one)", action='store_true')
    ...
    if system_name.lower() == 'fugaku' and args.summarize:
        # Keep only 1-Port recursive doubling (it's the only one existing in SoTa, the multiported version is ours)
        # I.e., remove the rows that start with "RECDOUB" and ends with something different from "1"
        df = df[~df["algo_name"].str.contains(r'^RECDOUB.*[^1]$', case=False, na=False)]

        algos_to_group = ['RECDOUB', 'SWING']
        for algo in algos_to_group:
            df_algo = df[df["algo_name"].str.contains(algo, case=False, na=False)]
            # Group df_algo by collective_type and buffer_size, by taking the minimum value of the mean column
            df_algo = df_algo.groupby(['collective_type', 'buffer_size']).min().reset_index()
            # Replace the algo_name column with algo
            df_algo['algo_name'] = algo
            # In df, remove all the rows containing algo in the algo_name column, and add df_algo rows to it
            df = df[~df["algo_name"].str.contains(algo, case=False, na=False)]
            df = pd.concat([df, df_algo], ignore_index=True)
