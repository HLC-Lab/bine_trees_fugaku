...
[ "${FUNC_NAME}" == "swing_reduce_mpi" ] || { echo "ERROR: Wrong function called for ${LIBSWING_REDUCE_ALGO_FAMILY} ${LIBSWING_REDUCE_ALGO} ${LIBSWING_REDUCE_ALGO_LAYER}: ${FUNC_NAME}"; exit 1; }
...
export LIBSWING_REDUCE_ALGO_LAYER="MPI" 
FUNC_NAME=$(mpirun -n 4 --oversubscribe ./bench/bench_validate ${COLLECTIVE} "INT32" "131072" "4" 2>/dev/null | grep "func_called" | cut -d ':' -f 2 | head -n 1 | tr -d ' ')
[ "${FUNC_NAME}" == "swing_reduce_redscat_gather_mpi" ] || { echo "ERROR: Wrong function called for ${LIBSWING_REDUCE_ALGO_FAMILY} ${LIBSWING_REDUCE_ALGO} ${LIBSWING_REDUCE_ALGO_LAYER}: ${FUNC_NAME}"; exit 1; }
...
export LIBSWING_REDUCE_ALGO_LAYER="UTOFU" 
FUNC_NAME=$(mpirun -n 4 --oversubscribe ./bench/bench_validate ${COLLECTIVE} "INT32" "131072" "4" 2>/dev/null | grep "func_called" | cut -d ':' -f 2 | head -n 1 | tr -d ' ')
[ "${FUNC_NAME}" == "swing_reduce_redscat_gather_utofu" ] || { echo "ERROR: Wrong function called for ${LIBSWING_REDUCE_ALGO_FAMILY} ${LIBSWING_REDUCE_ALGO} ${LIBSWING_REDUCE_ALGO_LAYER}: ${FUNC_NAME}"; exit 1; }
    ...
    do
        ...
        do
            ...
            do  
                ...
                for ITERATIONS in 4
                do
                    export LIBSWING_REDUCE_ALGO=${ALGO}
                    export LIBSWING_REDUCE_ALGO_FAMILY=${FAMILY}
                    export LIBSWING_REDUCE_ALGO_LAYER="MPI"
                    for N in 4 8 64 
                    do
