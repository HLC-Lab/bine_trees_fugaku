#!/bin/bash
declare -a ALGORITHMS=("VEC_DOUBLING_BLOCKS" "VEC_DOUBLING_CONT_SEND" "VEC_DOUBLING_CONT_PERMUTE")
...
[ "${FUNC_NAME}" == "bine_allgather_mpi_contiguous" ] || { echo "ERROR: Wrong function called for ${LIBBINE_ALLGATHER_ALGO_FAMILY} ${LIBBINE_ALLGATHER_ALGO} ${LIBBINE_ALLGATHER_ALGO_LAYER}: ${FUNC_NAME}"; exit 1; }
...
FUNC_NAME=$(mpirun -n 4 --oversubscribe ./bench/bench_validate ${COLLECTIVE} "INT32" "131072" "4" 2>/dev/null | grep "func_called" | cut -d ':' -f 2 | head -n 1 | tr -d ' ')
[ "${FUNC_NAME}" == "bine_allgather_utofu_contiguous_threads" ] || { echo "ERROR: Wrong function called for ${LIBBINE_ALLGATHER_ALGO_FAMILY} ${LIBBINE_ALLGATHER_ALGO} ${LIBBINE_ALLGATHER_ALGO_LAYER}: ${FUNC_NAME}"; exit 1; }
...
export LIBBINE_ALLGATHER_ALGO_LAYER="MPI" 
FUNC_NAME=$(mpirun -n 4 --oversubscribe ./bench/bench_validate ${COLLECTIVE} "INT32" "131072" "4" 2>/dev/null | grep "func_called" | cut -d ':' -f 2 | head -n 1 | tr -d ' ')
[ "${FUNC_NAME}" == "bine_allgather_send_mpi" ] || { echo "ERROR: Wrong function called for ${LIBBINE_ALLGATHER_ALGO_FAMILY} ${LIBBINE_ALLGATHER_ALGO} ${LIBBINE_ALLGATHER_ALGO_LAYER}: ${FUNC_NAME}"; exit 1; }
...
export LIBBINE_ALLGATHER_ALGO_LAYER="UTOFU" 
FUNC_NAME=$(mpirun -n 4 --oversubscribe ./bench/bench_validate ${COLLECTIVE} "INT32" "131072" "4" 2>/dev/null | grep "func_called" | cut -d ':' -f 2 | head -n 1 | tr -d ' ')
[ "${FUNC_NAME}" == "bine_allgather_send_utofu" ] || { echo "ERROR: Wrong function called for ${LIBBINE_ALLGATHER_ALGO_FAMILY} ${LIBBINE_ALLGATHER_ALGO} ${LIBBINE_ALLGATHER_ALGO_LAYER}: ${FUNC_NAME}"; exit 1; }
...
export LIBBINE_ALLGATHER_ALGO_LAYER="MPI" 
FUNC_NAME=$(mpirun -n 4 --oversubscribe ./bench/bench_validate ${COLLECTIVE} "INT32" "131072" "4" 2>/dev/null | grep "func_called" | cut -d ':' -f 2 | head -n 1 | tr -d ' ')
[ "${FUNC_NAME}" == "bine_allgather_blocks_mpi" ] || { echo "ERROR: Wrong function called for ${LIBBINE_ALLGATHER_ALGO_FAMILY} ${LIBBINE_ALLGATHER_ALGO} ${LIBBINE_ALLGATHER_ALGO_LAYER}: ${FUNC_NAME}"; exit 1; }
...
export LIBBINE_ALLGATHER_ALGO_LAYER="UTOFU" 
FUNC_NAME=$(mpirun -n 4 --oversubscribe ./bench/bench_validate ${COLLECTIVE} "INT32" "131072" "4" 2>/dev/null | grep "func_called" | cut -d ':' -f 2 | head -n 1 | tr -d ' ')
[ "${FUNC_NAME}" == "bine_allgather_blocks_utofu_threads" ] || { echo "ERROR: Wrong function called for ${LIBBINE_ALLGATHER_ALGO_FAMILY} ${LIBBINE_ALLGATHER_ALGO} ${LIBBINE_ALLGATHER_ALGO_LAYER}: ${FUNC_NAME}"; exit 1; }
    ...
    do
        ...
        do
            ...
            do  
                ...
                for ITERATIONS in 4
                do
                    export LIBBINE_ALLGATHER_ALGO=${ALGO}
                    export LIBBINE_ALLGATHER_ALGO_FAMILY=${FAMILY}
                    export LIBBINE_ALLGATHER_ALGO_LAYER="MPI"
                    for N in 4 8 64
                    do
                    ...
                    # Non p2 runs
#                    if [ $ALGO == "VEC_DOUBLING_BLOCKS" ]; then
