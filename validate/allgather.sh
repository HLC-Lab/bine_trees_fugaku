...
[ "${FUNC_NAME}" == "swing_allgather_mpi_contiguous" ] || { echo "ERROR: Wrong function called for ${LIBSWING_ALLGATHER_ALGO_FAMILY} ${LIBSWING_ALLGATHER_ALGO} ${LIBSWING_ALLGATHER_ALGO_LAYER}: ${FUNC_NAME}"; exit 1; }
...
export LIBSWING_ALLGATHER_ALGO_LAYER="MPI" 
FUNC_NAME=$(mpirun -n 4 --oversubscribe ./bench/bench_validate ${COLLECTIVE} "INT32" "131072" "4" 2>/dev/null | grep "func_called" | cut -d ':' -f 2 | head -n 1 | tr -d ' ')
[ "${FUNC_NAME}" == "swing_allgather_blocks_mpi" ] || { echo "ERROR: Wrong function called for ${LIBSWING_ALLGATHER_ALGO_FAMILY} ${LIBSWING_ALLGATHER_ALGO} ${LIBSWING_ALLGATHER_ALGO_LAYER}: ${FUNC_NAME}"; exit 1; }
    ...
    do
        ...
        do
            ...
            do  
                ...
                for ITERATIONS in 4
                do
                    export LIBSWING_ALLGATHER_ALGO=${ALGO}
                    export LIBSWING_ALLGATHER_ALGO_FAMILY=${FAMILY}
                    export LIBSWING_ALLGATHER_ALGO_LAYER="MPI"
                    for N in 4 8 64
                    do
