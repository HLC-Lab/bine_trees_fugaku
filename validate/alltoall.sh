...
[ "${FUNC_NAME}" == "bruck_alltoall" ] || { echo "ERROR: Wrong function called for ${LIBBINE_ALLTOALL_ALGO_FAMILY} ${LIBBINE_ALLTOALL_ALGO} ${LIBBINE_ALLTOALL_ALGO_LAYER}: ${FUNC_NAME}"; exit 1; }
...
export LIBBINE_ALLTOALL_ALGO_LAYER="MPI" 
FUNC_NAME=$(mpirun -n 4 --oversubscribe ./bench/bench_validate ${COLLECTIVE} "INT32" "131072" "4" 2>/dev/null | grep "func_called" | cut -d ':' -f 2 | head -n 1 | tr -d ' ')
[ "${FUNC_NAME}" == "bine_alltoall_mpi" ] || { echo "ERROR: Wrong function called for ${LIBBINE_ALLTOALL_ALGO_FAMILY} ${LIBBINE_ALLTOALL_ALGO} ${LIBBINE_ALLTOALL_ALGO_LAYER}: ${FUNC_NAME}"; exit 1; }
...
FUNC_NAME=$(mpirun -n 4 --oversubscribe ./bench/bench_validate ${COLLECTIVE} "INT32" "131072" "4" 2>/dev/null | grep "func_called" | cut -d ':' -f 2 | head -n 1 | tr -d ' ')
[ "${FUNC_NAME}" == "bine_alltoall_utofu" ] || { echo "ERROR: Wrong function called for ${LIBBINE_ALLTOALL_ALGO_FAMILY} ${LIBBINE_ALLTOALL_ALGO} ${LIBBINE_ALLTOALL_ALGO_LAYER}: ${FUNC_NAME}"; exit 1; }
    ...
    do
        ...
        do
            ...
            do  
                ...
                for ITERATIONS in 4
                do
                    export LIBBINE_ALLTOALL_ALGO=${ALGO}
                    export LIBBINE_ALLTOALL_ALGO_FAMILY=${FAMILY}
                    export LIBBINE_ALLTOALL_ALGO_LAYER="MPI"
