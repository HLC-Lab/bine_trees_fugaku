...
[ "${FUNC_NAME}" == "swing_scatter_mpi" ] || { echo "ERROR: Wrong function called for ${LIBSWING_SCATTER_ALGO_FAMILY} ${LIBSWING_SCATTER_ALGO} ${LIBSWING_SCATTER_ALGO_LAYER}: ${FUNC_NAME}"; exit 1; }
...
FUNC_NAME=$(mpirun -n 4 --oversubscribe ./bench/bench_validate ${COLLECTIVE} "INT32" "131072" "4" 2>/dev/null | grep "func_called" | cut -d ':' -f 2 | head -n 1 | tr -d ' ')
[ "${FUNC_NAME}" == "swing_scatter_utofu" ] || { echo "ERROR: Wrong function called for ${LIBSWING_SCATTER_ALGO_FAMILY} ${LIBSWING_SCATTER_ALGO} ${LIBSWING_SCATTER_ALGO_LAYER}: ${FUNC_NAME}"; exit 1; }
    ...
    do
        ...
        do
            ...
            do  
                ...
                for ITERATIONS in 4
                do
                    export LIBSWING_SCATTER_ALGO=${ALGO}
                    export LIBSWING_SCATTER_ALGO_FAMILY=${FAMILY}
                    export LIBSWING_SCATTER_ALGO_LAYER="MPI"
