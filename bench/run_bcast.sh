...
do
    ...
    #########################
    export LIBBINE_BCAST_ALGO_FAMILY="DEFAULT" 
    ...
    LIBBINE_BCAST_ALGO_FAMILY="DEFAULT" ${MPIRUN} -mca coll_select_show_decision_process 2 -mca coll_tuned_prealloc_size ${coll_tuned_prealloc_size} ${MPIRUN_MAP_BY_NODE_FLAG} ${MPIEXEC_OUT} -n ${p} ${MPIRUN_ADDITIONAL_FLAGS} ./bench ${COLLECTIVE} ${DATATYPE} ${n} 1
    ...
    start_time=$(date +%s)
    DEFAULT_ALGO="default"    
    LIBBINE_BCAST_ALGO_FAMILY="DEFAULT" ${MPIRUN} ${EXTRA_MCAS} -mca coll_tuned_prealloc_size ${coll_tuned_prealloc_size} ${MPIRUN_MAP_BY_NODE_FLAG} ${MPIEXEC_OUT} -n ${p} ${MPIRUN_ADDITIONAL_FLAGS} ./bench ${COLLECTIVE} ${DATATYPE} ${n} ${iterations}
    ALGO_FNAME=default-${DEFAULT_ALGO}
    ...
    do
        ...
        #            ALGO_FNAME=${LIBBINE_BCAST_ALGO_FAMILY}-${LIBBINE_BCAST_ALGO}-${LIBBINE_BCAST_ALGO_LAYER}-${SEGMENT_SIZE}-${PORTS}
        #            mv ${OUT_PREFIX}*.0 ${OUTPUT_DIR}/${EXP_ID}/${n}_${ALGO_FNAME}_${DATATYPE_lc}.csv; rm -f ${OUT_PREFIX}* 
        #            if [ -f ${ERR_PREFIX}*.0 ]; then mv ${ERR_PREFIX}*.0 ${OUTPUT_DIR}/${EXP_ID}/${n}_${ALGO_FNAME}_${DATATYPE_lc}.err; rm -f ${ERR_PREFIX}*; fi
        #        fi
        #    done
        ...
        if [ $n -ge $PORTS ]; then            
            ...
            export LIBBINE_BCAST_ALGO_FAMILY="BINE" 
            export LIBBINE_BCAST_ALGO_LAYER="UTOFU" 
            export LIBBINE_BCAST_ALGO="BINOMIAL_TREE_TMPBUF"
            for SEGMENT_SIZE in 0 4096 65536 1048576
            do                
                if [ $SEGMENT_SIZE -lt $msg_size ]; then
                    LIBBINE_SEGMENT_SIZE=${SEGMENT_SIZE} ${MPIRUN} ${MPIRUN_MAP_BY_NODE_FLAG} ${MPIEXEC_OUT} -n ${p} ${MPIRUN_ADDITIONAL_FLAGS} ./bench ${COLLECTIVE} ${DATATYPE} ${n} ${iterations}                    
...
#                fi
            ...
            #export LIBBINE_BCAST_ALGO_FAMILY="RECDOUB" 
            #export LIBBINE_BCAST_ALGO_LAYER="UTOFU" 
            #export LIBBINE_BCAST_ALGO="BINOMIAL_TREE"    
            #for SEGMENT_SIZE in 0 4096 65536 1048576
            #do                
