...
int main(int argc, char** argv){
  ...
  if(rank == 0){
    swing_utofu_comm_descriptor* desc = swing_utofu_setup_communication(0, 1, sbuffer, sizeof(char)*BUFFER_NELEM, rbuffer, sizeof(char)*BUFFER_NELEM);
    starttime = MPI_Wtime();      
    swing_utofu_isend(desc);
    swing_utofu_wait(desc);
    ...
    swing_utofu_destroy_communication(desc); 
  }else if(rank == 1){
    swing_utofu_comm_descriptor* desc = swing_utofu_setup_communication(0, 0, sbuffer, sizeof(char)*BUFFER_NELEM, rbuffer, sizeof(char)*BUFFER_NELEM);
    starttime = MPI_Wtime();      
    swing_utofu_isend(desc);
    swing_utofu_wait(desc);
    endtime_tofu = MPI_Wtime();          
