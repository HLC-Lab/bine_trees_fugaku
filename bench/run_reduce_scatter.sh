...
do
    ...
    #########################
    export LIBBINE_REDUCE_SCATTER_ALGO_FAMILY="DEFAULT" 
    export LIBBINE_REDUCE_SCATTER_ALGO_LAYER="MPI" 
    ...
    LIBBINE_REDUCE_SCATTER_ALGO_FAMILY="DEFAULT" ${MPIRUN} -mca coll_select_show_decision_process 2 -mca coll_tuned_prealloc_size ${coll_tuned_prealloc_size} ${MPIRUN_MAP_BY_NODE_FLAG} ${MPIEXEC_OUT} -n ${p} ${MPIRUN_ADDITIONAL_FLAGS} ./bench ${COLLECTIVE} ${DATATYPE} ${actual_count} 1
    ...
    start_time=$(date +%s)
    DEFAULT_ALGO="default"    
    LIBBINE_REDUCE_SCATTER_ALGO_FAMILY="DEFAULT" ${MPIRUN} ${EXTRA_MCAS} -mca coll_tuned_prealloc_size ${coll_tuned_prealloc_size} ${MPIRUN_MAP_BY_NODE_FLAG} ${MPIEXEC_OUT} -n ${p} ${MPIRUN_ADDITIONAL_FLAGS} ./bench ${COLLECTIVE} ${DATATYPE} ${actual_count} ${iterations}
    ALGO_FNAME=default-${DEFAULT_ALGO}
    ...
    for DEFAULT_ALGO in "recursive_halving" "ring" # "non-overlapping" 
    do        
        export LIBBINE_REDUCE_SCATTER_ALGO_FAMILY="DEFAULT" 
        ${MPIRUN} ${EXTRA_MCAS}  -mca coll_tuned_prealloc_size ${coll_tuned_prealloc_size} -mca coll_select_reduce_scatter_algorithm ${DEFAULT_ALGO} ${MPIRUN_MAP_BY_NODE_FLAG} ${MPIEXEC_OUT} -n ${p} ${MPIRUN_ADDITIONAL_FLAGS} ./bench ${COLLECTIVE} ${DATATYPE} ${actual_count} ${iterations}
    ...
    do
        if [ $actual_count -ge $PORTS ]; then
            ...
            # Run bine
            export LIBBINE_REDUCE_SCATTER_ALGO_FAMILY="BINE" 
            export LIBBINE_REDUCE_SCATTER_ALGO_LAYER="UTOFU" 
            export LIBBINE_REDUCE_SCATTER_ALGO="VEC_HALVING_CONT_PERMUTE"    
            ...
            do                
                if [ $SEGMENT_SIZE -lt $total_msg_size ]; then
                    ...
                    ALGO_FNAME=${LIBBINE_REDUCE_SCATTER_ALGO_FAMILY}-${LIBBINE_REDUCE_SCATTER_ALGO}-${LIBBINE_REDUCE_SCATTER_ALGO_LAYER}-${SEGMENT_SIZE}-${PORTS}
                    mv ${OUT_PREFIX}*.0 ${OUTPUT_DIR}/${EXP_ID}/${n}_${ALGO_FNAME}_${DATATYPE_lc}.csv; rm -f ${OUT_PREFIX}* 
                    if [ -f ${ERR_PREFIX}*.0 ]; then mv ${ERR_PREFIX}*.0 ${OUTPUT_DIR}/${EXP_ID}/${n}_${ALGO_FNAME}_${DATATYPE_lc}.err; rm -f ${ERR_PREFIX}*; fi
                fi
            done        
            ...
            export LIBBINE_REDUCE_SCATTER_ALGO_FAMILY="RING" 
            export LIBBINE_REDUCE_SCATTER_ALGO_LAYER="UTOFU" 
            export LIBBINE_REDUCE_SCATTER_ALGO="VEC_HALVING_CONT_PERMUTE"    
            for SEGMENT_SIZE in 0 #4096 65536 1048576
            do                
    ...
    #export LIBBINE_NUM_PORTS=${PORTS}
    ## Run recdoub
    #export LIBBINE_REDUCE_SCATTER_ALGO_FAMILY="RECDOUB" 
    #export LIBBINE_REDUCE_SCATTER_ALGO_LAYER="UTOFU" 
    #export LIBBINE_REDUCE_SCATTER_ALGO="VEC_HALVING_CONT_PERMUTE"    
    #for SEGMENT_SIZE in 0 #4096 65536 1048576
    #do                
